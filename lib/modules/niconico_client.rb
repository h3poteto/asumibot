...
class NiconicoClient
  ...
  def get_today_movies()
    ...
    @niconico_search.each do |keyword|
      ...
      results = nico.search(keyword, search: [:title, :description, :tags], size: 19, filters: [
        {type: :range,
          field: :start_time,
  ...
  def get_popular_movies
    @niconico_tags.each do |tag|
      ...
      results = nico.search(tag, search: [:tags_exact], sort_by: :last_comment_time, size: 15)
  ...
  def add_popular_niconicos(movie, priority)
    NiconicoPopular.create(title: Sanitize.clean(movie.title), url: @watch_path + movie.cmsid, description: Sanitize.clean(movie.description), priority: priority )
  end

  def add_today_niconicos(movie)
    TodayNiconico.create(title: Sanitize.clean(movie.title), url: @watch_path + movie.cmsid, description: Sanitize.clean(movie.description), priority: nil)
  end

  def add_niconicos(movie)
    NiconicoMovie.create(title: Sanitize.clean(movie.title), url: @watch_path + movie.cmsid, description: Sanitize.clean(movie.description), priority: nil )
  end
end
