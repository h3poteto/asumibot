...
RSpec.describe NewSerif, type: :model do
  ...
  describe 'when update' do
    context 'after create' do
      before do
        @attr = attributes_for(:new_serif)
        @serif = create(:new_serif)
        @serif.update_attributes(@attr)
      end
      subject { NewSerif.find(@serif.id) }
  ...
  describe 'when delete', :delete do
    subject { create(:new_serif) }
    it "should delete" do
      id = subject.id
      expect(subject.destroy).not_to be_falsey
      expect { NewSerif.find(id) }.to raise_error(ActiveRecord::RecordNotFound)
    end
  end
end
